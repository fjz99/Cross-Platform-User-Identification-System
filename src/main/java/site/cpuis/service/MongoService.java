...
//@Service
@Slf4j
public class MongoService<T> implements DataBaseService<T> {
...
//        query.addCriteria (Criteria.where ("id").gte (1).lte (22));
    ...
    public List<T> selectByEqualsGeneric(String collectionName, Class<T> clazz, String field, Object data) {
        //设置分页参数
        Query query = new Query ();
        //设置分页信息
        query.addCriteria (Criteria.where (field).is (data));
...
//        log.debug ("mongo insert {}", info);
    ...
    public <E> void deleteByEqualGeneric(E data, Class<T> clazz, String collectionName, String fieldName) {
        // 设置查询条件，当id=#{id}
        Query query = new Query ();
        query.addCriteria (Criteria.where (fieldName).is (data));
        // mongodb在删除对象的时候会判断对象类型，如果你不传入对象类型，只传入了集合名称，它是找不到的
